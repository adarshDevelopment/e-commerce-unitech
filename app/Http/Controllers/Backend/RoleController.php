...
class RoleController extends BackendBaseController
{
    ...
    protected $title;
    protected $base_route = 'backend.acl.role';
    protected $view_path = 'backend.acl.role';
    ...
    public function assignForm($id)
    {
        ...
        $data['role'] = $this->model->find($id);
        $data['modules'] = Module::all();
        //fetching data from permission_roles pivot table from the selected role it
        $role_permissions = $data['role']->permissions()->get();
        //creating an empty array to store the assigned permissions
        $assigned_permission = [];
        foreach($role_permissions as $rp){
            array_push($assigned_permission, $rp->id);
        }
        $data['assigned_permissions'] = $assigned_permission;
        //calling __loadDataToView function to return assign form page
        return view($this->__loadDataToView($this->view_path.'.assign_form'), compact('data'));
    }
    ...
    {
        $data['role'] = $this->model->find($request->role_id);
        $data['role']->permissions()->sync($request->permission_id);
        $request->session()->flash('success_message', 'Permissions assigned successfully!');
